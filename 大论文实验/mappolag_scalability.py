...
class MAPPO_Lagrangian:
    def __init__(self, state_dim, action_dim, n_agents, logger=None, device="cpu", episode_length=100, **kwargs):
        ...
        self.lagrangian_lr = float(kwargs.get("lagrangian_lr", 0.0001))
...
# ================================================================
# 3. 运行单个 Scalability 实验
# ================================================================
def run_scalability_episode(cost_limit, fixed_lambda, num_agents, epochs, steps_per_epoch, seed, device):
    try:
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            print(f"   > Ep {epoch}/{epochs} | AvgRet: {avg_ep_reward:.2f}")

    return np.mean(final_rewards) if final_rewards else 0.0


# ================================================================
# 4. 绘图：分组柱状图 (Reward版)
# ================================================================
def plot_grouped_bar_chart(agent_counts, results, scenarios, save_dir):
    ...
    for i, sc in enumerate(scenarios):
        ...
        for count in agent_counts:
            reward = results[count][sc['label']]
            means.append(reward)  # 【修改】直接使用 Reward，不取负数

        offset = (i - (n_bars - 1) / 2) * bar_width
        # 【修改】删除了 bar_label，仅保留柱子
        ax.bar(x_indices + offset, means, bar_width, label=sc['label'], color=colors[i], edgecolor='black', alpha=0.8)

    ax.set_xlabel('Number of Agents (Users)')
    # 【修改】Y轴标签改为 Reward
    ax.set_ylabel('Average Episode Reward')
    ax.set_title('Scalability Analysis: Reward vs. Number of Agents')
    ax.set_xticks(x_indices)
    ax.set_xticklabels([str(c) for c in agent_counts])
    ax.legend(loc='upper right')  # 图例位置根据需要调整
    ax.grid(True, axis='y', alpha=0.3)

    plt.tight_layout()
    save_path = os.path.join(save_dir, "scalability_reward_comparison.png")
...
def run_scalability_experiment(args):
    ...
    scenarios = [
        ...
        {"label": "Fix $\lambda=100$", "fixed_val": 5.0, "color": "tab:green"},
        {"label": "Fix $\lambda=50$", "fixed_val": 4.0, "color": "tab:purple"},
        {"label": "Fix $\lambda=1$", "fixed_val": 0.0, "color": "tab:blue"},
    ]

    device = "cuda" if torch.cuda.is_available() else "cpu"
    save_dir = "./logs/mec_scalability"
    os.makedirs(save_dir, exist_ok=True)

    results = {}

    print(f">>> 开始可扩展性实验 (Agents: {AGENT_COUNTS})")

    for n_agent in AGENT_COUNTS:
        print(f"\n========== Testing with {n_agent} Agents ==========")
        results[n_agent] = {}

        for sc in scenarios:
            print(f"Running: {sc['label']}...")

            final_reward = run_scalability_episode(
            ...
            results[n_agent][sc['label']] = final_reward
            print(f"--> {sc['label']} Finished. Final Reward: {final_reward:.2f}")

    print("\n>>> 所有实验结束，开始绘图...")
    plot_grouped_bar_chart(AGENT_COUNTS, results, scenarios, save_dir)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    # 默认 50 Epochs 用于快速验证，正式图建议 100+
