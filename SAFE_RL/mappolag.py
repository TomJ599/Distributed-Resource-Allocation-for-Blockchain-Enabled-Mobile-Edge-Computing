...
# project imports (require safepo package layout)
try:
    from safepo.common.logger import EpochLogger
except Exception as e:
    raise ImportError("无法导入 EpochLogger，请确认 safepo.common.logger 可用。原始异常: {}".format(e))
# 尝试导入算法实现（仓库的实现）
try:
    from safepo.algos.mappolag import MAPPO_Lagrangian
except Exception:
# 如果仓库中没有该模块/类，给出友好提示
    MAPPO_Lagrangian = None


# 导入你的 MEC 环境类（请确保 mec_env.py 在 safepo 包下）
try:
    from safepo.mec_env import MecBCEnv
except Exception as e:
    raise ImportError("无法导入 MecBCEnv（safepo.mec_env）。请确认文件 safepo/mec_env.py 存在并包含 MecBCEnv 类。原始异常: {}".format(e))
try:
...
def make_ma_mec_env(n_agents: int, **kwargs) -> MecBCEnv:
    """构建并返回 MEC 多智能体环境实例。
    传入的 kwargs 会被直接转发到 MecBCEnv 构造函数。
    """
    env = MecBCEnv(n_agents=n_agents, **kwargs)
    return env
...
# ================================================================
# 多组数据对比绘图函数 (已修改：增加线型区分)
# ================================================================
def plot_combined_shadow_curves(results_list, cost_limit, save_dir):
    """
    results_list: list of dict, 每个元素包含 {"df": dataframe, "label": str, "color": str}
    """
    ...
    for item in results_list:
        ...
        if df is not None:
            ...
            plt.plot(df["Episode"], df["SmoothCost"],
                     color=color, linewidth=linewidth, linestyle=linestyle,
                     label=label, alpha=1.0, zorder=zorder)

    # 绘制阈值线 (Limit=25)
    plt.axhline(y=cost_limit, color='black', linestyle=':', linewidth=2.0, alpha=0.6, label=f'Limit ({cost_limit})')

    plt.xlabel("Episode")
    plt.ylabel("Episode Cost")
    plt.title(f"Cost Comparison (Smoothed)")
    plt.legend(loc='upper right', frameon=True, shadow=True)
    plt.grid(True, alpha=0.15)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "combined_cost_smooth.png"), dpi=300)
    plt.close()

    # --- 2. Combined Reward Plot ---
    plt.figure(figsize=(12, 8))
    ...
    for item in results_list:
        ...
        if df is not None:
            # 同样应用线型逻辑，保持风格统一
            ls = '-' if "Adaptive" in label else ('--' if "100" in label else '-.')
            plt.plot(df["Episode"], df["RawReward"],
                     color=color, alpha=0.15, linewidth=0.5)
            plt.plot(df["Episode"], df["SmoothReward"],
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "combined_reward_shadow.png"), dpi=300)
    plt.close()
...
def run_single_experiment(cost_limit, fixed_lambda=None, custom_label=None, num_agents=4, epochs=500,
    ...
    logger.log(f"--- 实验开始: {exp_name} | Limit={cost_limit} ---")
    ...
    algo = MAPPO_Lagrangian(
        ...
        lagrangian_lr=0.0005,
    )

    if fixed_lambda is not None:
        algo.lambda_global = float(fixed_lambda)
        algo.lagrangian_lr = 0.0
        logger.log(f"Fixed Lambda Mode: Value = {fixed_lambda}")
    else:
        logger.log(f"Adaptive Lambda Mode")
    ...
    for epoch in range(epochs):
        ...
        while steps < steps_per_epoch:
            ...
            if done or ep_len >= 100:
                all_ep_rewards.append(ep_ret)
                all_ep_costs.append(ep_cost)
                total_episodes += 1
        ...
        print(f"{exp_name} | Ep {epoch} | Ret {mean_ret:.1f} | Cost {mean_cost:.1f} | Lam {algo.lambda_global:.2f}")
    ...
    df = pd.DataFrame({"Episode": np.arange(len(all_ep_rewards)), "RawReward": all_ep_rewards, "RawCost": all_ep_costs})
    SMOOTH_WIN = 1000
    df["SmoothReward"] = df["RawReward"].rolling(window=SMOOTH_WIN, min_periods=1).mean()
    df["SmoothCost"] = df["RawCost"].rolling(window=SMOOTH_WIN, min_periods=1).mean()
...
def run_comparison_experiment(args):
    ...
    scenarios = [
        ...
        {"label": "Fixed $\lambda=100$", "fixed_val": 100.0, "color": "tab:green"},
        {"label": "Fixed $\lambda=50$", "fixed_val": 50.0, "color": "tab:purple"},
        {"label": "Fixed $\lambda=1$", "fixed_val": 0.1, "color": "tab:blue"},
    ]

    TARGET_LIMIT = args.limit
    results_list = []

    print(f"\n>>> 启动对比实验 (Limit={TARGET_LIMIT})")
    ...
    plot_combined_shadow_curves(results_list, TARGET_LIMIT, save_dir)
    print(f"\n>>> 所有实验结束，对比图已保存至: {save_dir}")
...
if __name__ == "__main__":
    ...
    # 【修改点】将默认阈值设为 25.0
    parser.add_argument("--limit", type=float, default=19.0, help="Cost Limit")
